...
describe('plenary.popup', function()
    ...
    it('can apply a highlight', function()
        ...
        eq(
            'Normal:PopupColor1,EndOfBuffer:PopupColor1',
            vim.api.nvim_win_get_option(win_id, 'winhl')
        )
    end)
    ...
    it('can apply a border highlight', function()
        ...
        eq(
            'Normal:PopupColor2',
            vim.api.nvim_win_get_option(border_win_id, 'winhl')
        )
    end)
    ...
    describe('borderchars', function()
        local test_border = function(name, borderchars, expected)
            it(name, function()
                ...
                eq(
                    expected,
                    vim.api.nvim_buf_get_lines(border_bufnr, 0, -1, false)
                )
            end)
        end
        ...
        test_border(
            'can support multiple patterns inside the borderchars',
            { '-', '+' },
            {
                '+------------------------+',
                '-                        -',
