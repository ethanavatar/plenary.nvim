    ...
    describe(
        ...
        function() --------------------------------------------------
            ...
            it('downloads files to opts.output synchronously', function()
                local file =
                    'https://media2.giphy.com/media/bEMcuOG3hXVnihvB7x/giphy.gif'
            ...
            it('sends with basic-auth as string', function()
                ...
                res = curl.get(url, { auth = auth })
                assert(
                    not incl('authenticated.*true', res.body),
                    'it should fail'
                )
                eq(401, res.status)
            end)
    ...
    describe(
        ...
        function() --------------------------------------------------
            ...
            it('sends lua table', function()
                local res =
                    curl.post('https://jsonplaceholder.typicode.com/posts', {
                        body = {
                            title = 'Hello World',
                            body = '...',
                        },
    ...
    describe(
        ...
        function() --------------------------------------------------
            it('sends changes and get be back the new version.', function()
                ...
                local res =
                    curl.put('https://jsonplaceholder.typicode.com/posts/8', {
                        body = cha,
                    })
    ...
    describe(
        ...
        function() ------------------------------------------------
            it('sends changes and get be back the new version.', function()
                local cha = { title = 'New Title' }
                local res =
                    curl.patch('https://jsonplaceholder.typicode.com/posts/8', {
                        body = cha,
                    })
                eq(cha.title, vim.fn.json_decode(res.body).title)
                eq(200, res.status)
    ...
    describe(
        ...
        function() ------------------------------------------------
            it('sends delete request', function()
                local res =
                    curl.delete('https://jsonplaceholder.typicode.com/posts/8')
                eq(200, res.status)
            end)
        end
    )
