...
describe('strings', function()
    describe('strdisplaywidth', function()
        for _, case in ipairs({
            ...
            for _, ambiwidth in ipairs({ 'single', 'double' }) do
                ...
                local msg = ('ambiwidth = %s, ' .. item .. ' -> %d'):format(
                    ambiwidth,
                    case.str,
                ...
                it('lua: ' .. msg, function()
                    eq(
                        case.expected[ambiwidth],
                        strings.strdisplaywidth(case.str)
                    )
                end)
                it('vim: ' .. msg, function()
                    eq(
                        case.expected[ambiwidth],
                        vim.fn.strdisplaywidth(case.str)
                    )
                end)
    ...
    describe('truncate', function()
        for _, case in ipairs({
            ...
            {
                args = { 'abcde', 6, nil, 1 },
                expected = { single = 'abcde', double = 'abcde' },
            },
            {
                args = { 'abcde', 5, nil, 1 },
                expected = { single = 'abcde', double = 'abcde' },
            ...
            {
                args = { 'アイウエオ', 8, nil, 1 },
                expected = { single = 'アイウ…', double = 'アイウ…' },
            },
            {
                args = { '├─┤', 7, nil, 1 },
            ...
            {
                args = { 'アイウエオ', 8, nil, -1 },
                expected = { single = '…ウエオ', double = '…ウエオ' },
            },
            {
                args = { '├─┤', 7, nil, -1 },
                expected = { single = '├─┤', double = '├─┤' },
            ...
            {
                args = { 'abcde', 4, nil, 0 },
                expected = { single = 'a…de', double = 'a…e' },
            },
            {
                args = { 'アイウエオ', 11, nil, 0 },
                expected = {
            ...
            {
                args = { 'アイウエオ', 8, nil, 0 },
                expected = { single = 'ア…エオ', double = 'ア…エオ' },
            },
            {
                args = { '├─┤', 7, nil, 0 },
                expected = { single = '├─┤', double = '├─┤' },
            ...
            for _, ambiwidth in ipairs({ 'single', 'double' }) do
                local msg = ('ambiwidth = %s, direction = %s, [%s, %d] -> %s'):format(
                    ambiwidth,
                    (case.args[4] > 0) and 'right'
                        or (case.args[4] < 0) and 'left'
                        or 'middle',
                    case.args[1],
                    case.args[2],
                    case.expected[ambiwidth]
                ...
                it(msg, function()
                    ...
                    eq(
                        case.expected[ambiwidth],
                        strings.truncate(unpack(case.args))
                    )
                    vim.o.ambiwidth = original
                end)
            end
        ...
        for _, case in ipairs({
            ...
            for _, ambiwidth in ipairs({ 'single', 'double' }) do
                ...
                it(msg, function()
                    ...
                    eq(
                        case.expected[ambiwidth],
                        strings.align_str(unpack(case.args))
                    )
                    vim.o.ambiwidth = original
                end)
            end
