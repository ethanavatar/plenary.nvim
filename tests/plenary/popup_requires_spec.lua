...
describe('plenary.popup', function()
    ...
    local matches_any_import = function(line)
        ...
        if matched and not vim.startswith(matched, 'plenary.popup') then
            if not allowed_imports[matched] then
                return true,
                    string.format(
                        'Not an allowed import for popup: %s. Line: %s',
                        matched,
                        line
                    )
            end
    ...
    it('should match these kinds of patterns', function()
        ...
        eq(
            true,
            matches_any_import([[local x = require("plenary.module").something]])
        )
    end)

    it(
        'must not require anything other than Window and Border from plenary',
        function()
            local result =
                scandir.scan_dir('./lua/plenary/popup', { depth = 1 })
