...
describe('json', function()
    it('replace comments with whitespace', function()
        eq(
            json.json_strip_comments('//comment\n{"a":"b"}'),
            '         \n{"a":"b"}'
        )
        eq(
            json.json_strip_comments('/*//comment*/{"a":"b"}'),
            '             {"a":"b"}'
        )
        eq(
            json.json_strip_comments('{"a":"b"//comment\n}'),
            '{"a":"b"         \n}'
        )
        eq(
            json.json_strip_comments('{"a":"b"/*comment*/}'),
        ...
        eq(
            json.json_strip_comments('{"a"/*\n\n\ncomment\r\n*/:"b"}'),
            '{"a"  \n\n\n       \r\n  :"b"}'
        )
        eq(
            json.json_strip_comments('/*!\n * comment\n */\n{"a":"b"}'),
            '   \n          \n   \n{"a":"b"}'
        )
        eq(
            json.json_strip_comments('{/*comment*/"a":"b"}'),
    ...
    it('remove comments', function()
        ...
        eq(
            json.json_strip_comments('/*//comment*/{"a":"b"}', options),
            '{"a":"b"}'
        )
        eq(
            json.json_strip_comments('{"a":"b"//comment\n}', options),
        ...
        eq(
            json.json_strip_comments('{"a":"b"/*comment*/}', options),
            '{"a":"b"}'
        )
        eq(
            json.json_strip_comments('{"a"/*\n\n\ncomment\r\n*/:"b"}', options),
    ...
    it(
        'consider escaped slashes when checking for escaped string quote',
        function()
            eq(
                json.json_strip_comments('{"\\\\":"https://foobar.com"}'),
                '{"\\\\":"https://foobar.com"}'
            )
            eq(
                json.json_strip_comments('{"foo\\"":"https://foobar.com"}'),
    ...
    it('line endings - multi line block comment', function()
        eq(
            json.json_strip_comments('{"a":"b",/*c\nc2*/"x":"y"\n}'),
            '{"a":"b",   \n    "x":"y"\n}'
        )
        eq(
            json.json_strip_comments('{"a":"b",/*c\r\nc2*/"x":"y"\r\n}'),
