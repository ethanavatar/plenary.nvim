...
path.sep = (function()
    if jit then
        ...
        --if os ~= "windows" then
        if not S.is_windows() and not S.uses_shellslash() then
            return '/'
        else
            return '\\'
        end
...
local is_absolute = function(filename, sep)
    ...
    if S.is_windows() then
        ret = string.match(filename, '^[%a]:' .. _sep .. '.*$') ~= nil
    else
        ret = string.sub(filename, 1, 1) == sep
    end
...
local function _normalize_path(filename, cwd)
    ...
    local has = string.find(filename, path.sep .. '..', 1, true)
        or string.find(filename, '..' .. path.sep, 1, true)
...
function Path:absolute()
    if self:is_absolute() then
        ...
        return _normalize_path(
            self._absolute
                or table.concat({ self._cwd, self.filename }, self._sep),
            self._cwd
        )
    end
end
...
function Path:expand()
    ...
    elseif string.find(self.filename, '%$') then
        ...
        if val then
            expanded =
                string.gsub(string.gsub(self.filename, rep, val), '%$', '')
        else
            expanded = nil
        end
    else
...
function Path:normalize(cwd)
    ...
    if start == 1 then
        self.filename = '~'
            .. path.sep
            .. string.sub(self.filename, (finish + 1), -1)
    end
        ...
        if parents then
            ...
            for _, dir in ipairs(dirs) do
                if dir ~= '' then
                    ...
                    if processed == '' and S.is_windows() then
                        joined = dir
                    end
                    ...
                    if band(S_IF.REG, file_mode) then
                        error(
                            string.format(
                                '%s is a regular file so we can\'t mkdir it',
                                joined
                            )
                        )
...
function Path:copy(opts)
    ...
    if opts.recursive then
        ...
        for _, entry in ipairs(data) do
            ...
            local new_opts =
                vim.tbl_deep_extend('force', opts, { destination = new_dest })
            -- nil: not overriden if `override = false`
