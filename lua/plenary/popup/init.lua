...
local function add_position_config(win_opts, vim_options, default_opts)
    ...
    win_opts.width =
        utils.bounded(width, vim_options.minwidth, vim_options.maxwidth)
    win_opts.height =
        utils.bounded(height, vim_options.minheight, vim_options.maxheight)
...
function popup.create(what, vim_options)
    ...
    if dict_default(vim_options, 'posinvert', option_defaults) then
        ...
        elseif win_opts.anchor == 'SW' or win_opts.anchor == 'SE' then
            if
                win_opts.row - win_opts.height < 0
                and win_opts.row * 2 < vim.o.lines
            then
                -- Don't know why, but this is how vim adjusts it
                win_opts.row = win_opts.row + win_opts.height + 2
    ...
    if vim_options.wrap ~= nil then
        ...
        if vim_options.noautocmd then
            vim.cmd(
                string.format(
                    'noautocmd lua vim.api.nvim_set_option(%s, wrap, %s)',
                    win_id,
                    vim_options.wrap
                )
            )
    ...
    if vim_options.border then
        ...
        if
            type(vim_options.border) == 'boolean'
            or vim.tbl_isempty(vim_options.border)
        then
    ...
    if
        (should_show_border == nil or should_show_border)
        and vim_options.borderchars
    then
        should_show_border = true
        ...
        elseif #vim_options.borderchars == 8 then
            b_top, b_right, b_bot, b_left, b_topleft, b_topright, b_botright, b_botleft =
                unpack(vim_options.borderchars)
        else
    ...
    if vim_options.highlight then
        vim.api.nvim_win_set_option(
            ...
            'winhl',
            string.format(
                'Normal:%s,EndOfBuffer:%s',
                vim_options.highlight,
                vim_options.highlight
            )
        )
    end
    ...
    if should_enter then
        ...
        if vim_options.noautocmd then
            vim.cmd(
                'noautocmd lua vim.api.nvim_set_current_win(' .. win_id .. ')'
            )
        else
    ...
    if vim_options.callback then
        ...
        vim.api.nvim_buf_set_keymap(
            ...
            'n',
            '<CR>',
            '<cmd>lua require"plenary.popup".execute_callback('
                .. bufnr
                .. ')<CR>',
            { noremap = true }
        )
