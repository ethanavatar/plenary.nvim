        ...
        if p:exists() then
            ...
            for l in p:iter() do
                ...
                if not (prefix == '' or prefix == '#') then
                    ...
                    if el ~= '' then
                        table.insert(
                            negated and patterns[v].negated
                                or patterns[v].ignored,
                            el
                        )
                    end
                end
...
m.scan_dir = function(path, opts)
    ...
    local gitignore = opts.respect_gitignore and make_gitignore(base_paths)
        or nil
    local match_search_pat = opts.search_pattern
            and gen_search_pat(opts.search_pattern)
        or nil
    ...
    for i = #base_paths, 1, -1 do
        if uv.fs_access(base_paths[i], 'X') == false then
            if not F.if_nil(opts.silent, false, opts.silent) then
                print(
                    string.format(
                        '%s is not accessible by the current user!',
                        base_paths[i]
                    )
                )
            end
        ...
        if not users_tbl and not groups_tbl then
            ...
            return function(...)
                ...
                for k = 5, 6 do
                    ...
                    table.insert(
                        section,
                        { start_index = cur_index, end_index = end_index }
                    )
