...
function Job:_execute()
    ...
    self.handle, self.pid =
        uv.spawn(options.command, options, shutdown_factory(self, options))
    ...
    if self.enable_handlers then
        self._stdout_reader =
            on_output(self, '_stdout_results', self._user_on_stdout)
        self.stdout:read_start(self._stdout_reader)

        self._stderr_reader =
            on_output(self, '_stderr_results', self._user_on_stderr)
