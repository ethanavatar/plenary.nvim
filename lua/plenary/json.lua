...
function M.json_strip_comments(jsonString, options)
    options = options or {}
    local strip = options.whitespace == false and stripWithoutWhitespace
        or stripWithWhitespace
    ...
    for i = 1, #jsonString, 1 do
        if skip then
            ...
            if not insideString then
                if
                    not insideComment
                    and currentCharacter .. nextCharacter == '//'
                then
    ...
    return result
        .. (
            insideComment and strip(slice(jsonString, offset))
            or slice(jsonString, offset)
        )
end
