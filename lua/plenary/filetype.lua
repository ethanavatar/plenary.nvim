...
filetype.add_table = function(new_filetypes)
    ...
    for k, v in pairs(new_keys) do
        error(
            debug.traceback(
                'Invalid key / value:' .. tostring(k) .. ' / ' .. tostring(v)
            )
        )
    end

    if new_filetypes.extension then
        filetype_table.extension = vim.tbl_extend(
            'force',
            filetype_table.extension,
            new_filetypes.extension
        )
    end

    if new_filetypes.file_name then
        filetype_table.file_name = vim.tbl_extend(
            'force',
            filetype_table.file_name,
            new_filetypes.file_name
        )
    end

    if new_filetypes.shebang then
        filetype_table.shebang = vim.tbl_extend(
            'force',
            filetype_table.shebang,
            new_filetypes.shebang
        )
    end
end
...
filetype.add_file = function(filename)
    local filetype_files = vim.api.nvim_get_runtime_file(
        string.format('data/plenary/filetypes/%s.lua', filename),
        true
    )
