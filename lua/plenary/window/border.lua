...
local create_horizontal_line = function(
    title,
    pos,
    width,
    left_char,
    ...
    if title_len ~= 0 then
        ...
        local r_start = string.len(left_char)
            + math.max(left_start, 0) * string.len(mid_char)
        ranges = { { r_start, r_start + string.len(title) } }
    end
    return horizontal_line, ranges
end

function Border._create_lines(
    content_win_id,
    content_win_options,
    border_win_options
)
    ...
    local right_enabled = thickness.right == 1
        and content_pos[2] + content_width < vim.o.columns
    ...
    local titles = type(border_win_options.title) == 'string'
            and { { pos = 'N', text = border_win_options.title } }
            ...
            if string.find(title.pos, 'S') then
                ...
                for _, r in pairs(bot_ranges) do
                    table.insert(
                        ranges,
                        {
                            content_win_options.height + thickness.top,
                            r[1],
                            r[2],
                        }
                    )
...
function Border:move(content_win_options, border_win_options)
    ...
    local nvim_win_config =
        self:__align_calc_config(content_win_options, border_win_options)
...
function Border:new(
    ...
    assert(
        type(content_win_id) == 'number',
        'Must supply a valid win_id. It\'s possible you forgot to call with \':\''
    )
    ...
    local nvim_win_config =
        Border.__align_calc_config(obj, content_win_options, border_win_options)
    ...
    set_title_highlights(
        obj.bufnr,
        obj.title_ranges,
        obj._border_win_options.titlehighlight
    )
